...
void cm_decoder(ARGS) 
{
	Digital_State_t *_op_y, *_op_y_old;
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/decoder/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
	...
	snprintf(send_data,sizeof(send_data),"sel:%s", temp_sel);

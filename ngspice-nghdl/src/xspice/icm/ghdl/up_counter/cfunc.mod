...
void cm_up_counter(ARGS) 
{
	...
	char temp_CLR[1024];
		...
		for(Ii=0;Ii<PORT_SIZE(CLR);Ii++)
		{
			LOAD(CLR[Ii])=PARAM(input_load); 
		}
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/up_counter/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
	...
	snprintf(send_data,sizeof(send_data),"Clock:%s,CLR:%s", temp_Clock,temp_CLR);

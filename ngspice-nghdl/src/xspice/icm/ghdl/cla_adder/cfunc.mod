...
void cm_cla_adder(ARGS) 
{
	Digital_State_t *_op_S, *_op_S_old;
	Digital_State_t *_op_cout, *_op_cout_old;
	...
	char temp_cin[1024];
		...
		_op_cout = _op_cout_old = (Digital_State_t *) cm_event_get_ptr(1,0);
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/cla_adder/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
		...
		_op_cout = (Digital_State_t *) cm_event_get_ptr(1,1);
		_op_cout_old = (Digital_State_t *) cm_event_get_ptr(1,2);
	}
	...
	snprintf(send_data,sizeof(send_data),"A:%s,B:%s,cin:%s", temp_A,temp_B,temp_cin);
        	...
        	if((key_iter=strstr(recv_data, "S:")) != NULL)
        	{
        		while(*key_iter++ != ':');
	...
	/* Scheduling event and processing them */
        	if((key_iter=strstr(recv_data, "cout:")) != NULL)
        	{
        		while(*key_iter++ != ':');
        		for(Ii=0;*key_iter != ';';Ii++,key_iter++)
        		{
			...
			else
			{
				...
				printf("Client-Unknown value return \n");
        			...
        			if(ANALYSIS == DC)
			{
        				OUTPUT_STATE(cout[Ii]) = _op_cout[Ii];
        			}
			else if(_op_cout[Ii] != _op_cout_old[Ii])
        			{
				OUTPUT_STATE(cout[Ii]) = _op_cout[Ii];
        				OUTPUT_DELAY(cout[Ii]) = ((_op_cout[Ii] == ZERO) ? PARAM(fall_delay) : PARAM(rise_delay));
        			}
			else
			{
        				OUTPUT_CHANGED(cout[Ii]) = FALSE;
			}
        			OUTPUT_STRENGTH(cout[Ii]) = STRONG;
        		}
        	}
	close(socket_fd);

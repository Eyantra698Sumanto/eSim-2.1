...
void cm_design(ARGS) 
{
	...
	char temp_S[1024];
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/design/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
	...
	snprintf(send_data,sizeof(send_data),"D0:%s,D1:%s,S:%s", temp_D0,temp_D1,temp_S);

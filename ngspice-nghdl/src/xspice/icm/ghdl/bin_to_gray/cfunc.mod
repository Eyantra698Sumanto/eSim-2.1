...
void cm_bin_to_gray(ARGS) 
{
	Digital_State_t *_op_G, *_op_G_old;
		...
		int Ii;
		for(Ii=0;Ii<PORT_SIZE(bin);Ii++)
		{
			LOAD(bin[Ii])=PARAM(input_load); 
		}

		/*Retrieve Storage for output*/
		_op_G = _op_G_old = (Digital_State_t *) cm_event_get_ptr(0,0);
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/bin_to_gray/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
		_op_G = (Digital_State_t *) cm_event_get_ptr(0,0);
		_op_G_old = (Digital_State_t *) cm_event_get_ptr(0,1);
	}
	...
	snprintf(send_data,sizeof(send_data),"bin:%s", temp_bin);
        	...
        	if((key_iter=strstr(recv_data, "G:")) != NULL)
        	{
        		while(*key_iter++ != ':');
			...
			else
			{
				...
				printf("Client-Unknown value return \n");
        			...
        			if(ANALYSIS == DC)
			{
        				OUTPUT_STATE(G[Ii]) = _op_G[Ii];
        			}
			else if(_op_G[Ii] != _op_G_old[Ii])
        			{
				OUTPUT_STATE(G[Ii]) = _op_G[Ii];
        				OUTPUT_DELAY(G[Ii]) = ((_op_G[Ii] == ZERO) ? PARAM(fall_delay) : PARAM(rise_delay));
        			}
			else
			{
        				OUTPUT_CHANGED(G[Ii]) = FALSE;
			}
        			OUTPUT_STRENGTH(G[Ii]) = STRONG;
        		}
        	}
	close(socket_fd);

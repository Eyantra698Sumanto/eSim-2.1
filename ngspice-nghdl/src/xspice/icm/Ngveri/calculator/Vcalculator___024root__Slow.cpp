// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See Vcalculator.h for the primary calling header

#include "Vcalculator___024root.h"
#include "Vcalculator__Syms.h"

//==========


void Vcalculator___024root___ctor_var_reset(Vcalculator___024root* vlSelf);

Vcalculator___024root::Vcalculator___024root(const char* _vcname__)
    : VerilatedModule(_vcname__)
 {
    // Reset structure values
    Vcalculator___024root___ctor_var_reset(this);
}

void Vcalculator___024root::__Vconfigure(Vcalculator__Syms* _vlSymsp, bool first) {
    if (false && first) {}  // Prevent unused
    this->vlSymsp = _vlSymsp;
}

Vcalculator___024root::~Vcalculator___024root() {
}

void Vcalculator___024root___eval_initial(Vcalculator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vcalculator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vcalculator___024root___eval_initial\n"); );
}

void Vcalculator___024root___combo__TOP__1(Vcalculator___024root* vlSelf);

void Vcalculator___024root___eval_settle(Vcalculator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vcalculator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vcalculator___024root___eval_settle\n"); );
    // Body
    Vcalculator___024root___combo__TOP__1(vlSelf);
}

void Vcalculator___024root___final(Vcalculator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vcalculator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vcalculator___024root___final\n"); );
}

void Vcalculator___024root___ctor_var_reset(Vcalculator___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vcalculator__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vcalculator___024root___ctor_var_reset\n"); );
    // Body
    vlSelf->astr = VL_RAND_RESET_I(14);
    vlSelf->res = VL_RAND_RESET_I(4);
    vlSelf->seven_out = VL_RAND_RESET_I(7);
}

...
{   
    ...
    if(INIT)
    {
        ...
        char path[1000]="/home/sumanto/ngspice-nghdl/src/xspice/icm/Ngveri/top/input";
        char ch=(char)(PARAM(instance_id)+48);    
        strncat(path,&ch,1);
        strcat(path,".txt");
        fclose(fopen(path,"w+"));
        fclose(fopen("/home/sumanto/ngspice-nghdl/src/xspice/icm/Ngveri/top/output.txt","r"));
    ...
    temp_clk[Ii]='\0';
    char pathio[1000]="/home/sumanto/ngspice-nghdl/src/xspice/icm/Ngveri/top/input";
    char ch=(char)(PARAM(instance_id)+48);    
    strncat(pathio,&ch,1);
    strcat(pathio,".txt");
    //printf("%s",path);
    ...
    fprintf(fpout,"%s",temp_reset);

    fclose(fpout);
    fptemp = fopen(pathio,"r");
    fscanf(fptemp,"%s", temp_data);
    fclose(fptemp);
    fpout = fopen("/home/sumanto/ngspice-nghdl/src/xspice/icm/Ngveri/top/input.txt","w+");
    fprintf(fpout,"%s", temp_data);
    fclose(fpout);
    system("/home/sumanto/ngspice-nghdl/src/xspice/icm/Ngveri/top/obj_dir/Vtop");
    fpin=fopen("/home/sumanto/ngspice-nghdl/src/xspice/icm/Ngveri/top/output.txt","r");
    fscanf(fpin,"%s", recv_data);
    fclose(fpin);
    printf("%lf %s %s %s %d\n",TIME,temp_clk,temp_reset,recv_data,count);

...
void cm_jtransmissiongate(ARGS) 
{
    ...
    {   
        ...
        port_control=PORT_SIZE(control);
		...
		for(Ii=0;Ii<PORT_SIZE(control);Ii++)
		{
			LOAD(control[Ii])=PARAM(input_load); 
		}
    ...
    for(Ii=0;Ii<PORT_SIZE(control);Ii++)
    {
        if( INPUT_STATE(control[Ii])==ZERO )
        {
            temp_control[Ii]=0;            }
        else
        {

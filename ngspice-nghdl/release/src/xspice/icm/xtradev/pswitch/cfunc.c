                                          ...
                                          inputs, outputs, etc.     */
{
    ...
    if ( mif_private->param[2]->element[0].bvalue == MIF_TRUE ) {   /* Logarithmic Variation in 'R' */
        ...
        int outOfLimit = 0;
        if (cntl_on > cntl_off) {
            inmean = ((mif_private->conn[0]->port[0]->input.rvalue - mif_private->param[0]->element[0].rvalue) / (mif_private->param[1]->element[0].rvalue - mif_private->param[0]->element[0].rvalue)) - cntl_mean;
            ...
            inmean = (mif_private->param[1]->element[0].rvalue - (mif_private->conn[0]->port[0]->input.rvalue) / (mif_private->param[0]->element[0].rvalue - mif_private->param[1]->element[0].rvalue)) - cntl_mean;
            if (mif_private->conn[0]->port[0]->input.rvalue < cntl_on) {
                r = r_on;
                outOfLimit = 1;
            }

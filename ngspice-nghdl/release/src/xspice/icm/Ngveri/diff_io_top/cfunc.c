#include "ngspice/cm.h"
extern void cm_diff_io_top(Mif_Private_t *);
/* This is cfunc.mod file auto generated by gen_con_info.py
        Developed by Sumanto Kar at IIT Bombay */

                
        #include <stdio.h>
        #include <math.h>
        #include <string.h>
        #include "sim_main_diff_io_top.h"

        
void cm_diff_io_top(Mif_Private_t *mif_private) 
{
	Digital_State_t *_op_tx_out, *_op_tx_out_old;
	Digital_State_t *_op_rx_locked, *_op_rx_locked_old;
	Digital_State_t *_op_tx_outclock, *_op_tx_outclock_old;

    static int inst_count=0;
    int count=0;
        
    if(mif_private->circuit.init)
    {   
        inst_count++;
        mif_private->param[0]->element[0].rvalue=inst_count;
        foodiff_io_top(0,inst_count);
        /* Allocate storage for output ports and set the load for input ports */

        
        port_rx_in=mif_private->conn[0]->size;

        port_rx_inclock=mif_private->conn[1]->size;

        port_rx_data_align=mif_private->conn[2]->size;

        port_tx_out=mif_private->conn[3]->size;

        port_rx_locked=mif_private->conn[4]->size;

        port_tx_outclock=mif_private->conn[5]->size;
		cm_event_alloc(0,2*sizeof(Digital_State_t));
		cm_event_alloc(1,1*sizeof(Digital_State_t));
		cm_event_alloc(2,1*sizeof(Digital_State_t));
		/* set the load for input ports. */
		int Ii;
		for(Ii=0;Ii<mif_private->conn[0]->size;Ii++)
		{
			mif_private->conn[0]->port[Ii]->load=mif_private->param[1]->element[0].rvalue; 
		}
		for(Ii=0;Ii<mif_private->conn[1]->size;Ii++)
		{
			mif_private->conn[1]->port[Ii]->load=mif_private->param[1]->element[0].rvalue; 
		}
		for(Ii=0;Ii<mif_private->conn[2]->size;Ii++)
		{
			mif_private->conn[2]->port[Ii]->load=mif_private->param[1]->element[0].rvalue; 
		}

		/*Retrieve Storage for output*/
		_op_tx_out = _op_tx_out_old = (Digital_State_t *) cm_event_get_ptr(0,0);
		_op_rx_locked = _op_rx_locked_old = (Digital_State_t *) cm_event_get_ptr(1,0);
		_op_tx_outclock = _op_tx_outclock_old = (Digital_State_t *) cm_event_get_ptr(2,0);


	}
	else
	{
		_op_tx_out = (Digital_State_t *) cm_event_get_ptr(0,0);
		_op_tx_out_old = (Digital_State_t *) cm_event_get_ptr(0,1);
		_op_rx_locked = (Digital_State_t *) cm_event_get_ptr(1,1);
		_op_rx_locked_old = (Digital_State_t *) cm_event_get_ptr(1,2);
		_op_tx_outclock = (Digital_State_t *) cm_event_get_ptr(2,2);
		_op_tx_outclock_old = (Digital_State_t *) cm_event_get_ptr(2,3);
	}

	//Formating data for sending it to client
	int Ii;
	count=(int)mif_private->param[0]->element[0].rvalue;

    for(Ii=0;Ii<mif_private->conn[0]->size;Ii++)
    {
        if( ((Digital_t*)(mif_private->conn[0]->port[Ii]->input.pvalue))->state==ZERO )
        {
            temp_rx_in[Ii]=0;            }
        else
        {
            temp_rx_in[Ii]=1;
        }
            }
    for(Ii=0;Ii<mif_private->conn[1]->size;Ii++)
    {
        if( ((Digital_t*)(mif_private->conn[1]->port[Ii]->input.pvalue))->state==ZERO )
        {
            temp_rx_inclock[Ii]=0;            }
        else
        {
            temp_rx_inclock[Ii]=1;
        }
            }
    for(Ii=0;Ii<mif_private->conn[2]->size;Ii++)
    {
        if( ((Digital_t*)(mif_private->conn[2]->port[Ii]->input.pvalue))->state==ZERO )
        {
            temp_rx_data_align[Ii]=0;            }
        else
        {
            temp_rx_data_align[Ii]=1;
        }
            }
	foodiff_io_top(1,count);

	/* Scheduling event and processing them */
    for(Ii=0;Ii<mif_private->conn[3]->size;Ii++)
    {
        if(temp_tx_out[Ii]==0)
        {
            _op_tx_out[Ii]=ZERO;
            }
        else if(temp_tx_out[Ii]==1)
        {
            _op_tx_out[Ii]=ONE;
            }
        else
        {
            printf("Unknown value\n");
                }

        if(mif_private->circuit.anal_type == DC)
        {
            ((Digital_t*)(mif_private->conn[3]->port[Ii]->output.pvalue))->state = _op_tx_out[Ii];
            }
        else if(_op_tx_out[Ii] != _op_tx_out_old[Ii])
        {
            ((Digital_t*)(mif_private->conn[3]->port[Ii]->output.pvalue))->state = _op_tx_out[Ii];
            mif_private->conn[3]->port[Ii]->delay = ((_op_tx_out[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
            }
        else
        {
            mif_private->conn[3]->port[Ii]->changed = FALSE;
            }
        ((Digital_t*)(mif_private->conn[3]->port[Ii]->output.pvalue))->strength = STRONG;
    }
	/* Scheduling event and processing them */
    for(Ii=0;Ii<mif_private->conn[4]->size;Ii++)
    {
        if(temp_rx_locked[Ii]==0)
        {
            _op_rx_locked[Ii]=ZERO;
            }
        else if(temp_rx_locked[Ii]==1)
        {
            _op_rx_locked[Ii]=ONE;
            }
        else
        {
            printf("Unknown value\n");
                }

        if(mif_private->circuit.anal_type == DC)
        {
            ((Digital_t*)(mif_private->conn[4]->port[Ii]->output.pvalue))->state = _op_rx_locked[Ii];
            }
        else if(_op_rx_locked[Ii] != _op_rx_locked_old[Ii])
        {
            ((Digital_t*)(mif_private->conn[4]->port[Ii]->output.pvalue))->state = _op_rx_locked[Ii];
            mif_private->conn[4]->port[Ii]->delay = ((_op_rx_locked[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
            }
        else
        {
            mif_private->conn[4]->port[Ii]->changed = FALSE;
            }
        ((Digital_t*)(mif_private->conn[4]->port[Ii]->output.pvalue))->strength = STRONG;
    }
	/* Scheduling event and processing them */
    for(Ii=0;Ii<mif_private->conn[5]->size;Ii++)
    {
        if(temp_tx_outclock[Ii]==0)
        {
            _op_tx_outclock[Ii]=ZERO;
            }
        else if(temp_tx_outclock[Ii]==1)
        {
            _op_tx_outclock[Ii]=ONE;
            }
        else
        {
            printf("Unknown value\n");
                }

        if(mif_private->circuit.anal_type == DC)
        {
            ((Digital_t*)(mif_private->conn[5]->port[Ii]->output.pvalue))->state = _op_tx_outclock[Ii];
            }
        else if(_op_tx_outclock[Ii] != _op_tx_outclock_old[Ii])
        {
            ((Digital_t*)(mif_private->conn[5]->port[Ii]->output.pvalue))->state = _op_tx_outclock[Ii];
            mif_private->conn[5]->port[Ii]->delay = ((_op_tx_outclock[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
            }
        else
        {
            mif_private->conn[5]->port[Ii]->changed = FALSE;
            }
        ((Digital_t*)(mif_private->conn[5]->port[Ii]->output.pvalue))->strength = STRONG;
    }

}
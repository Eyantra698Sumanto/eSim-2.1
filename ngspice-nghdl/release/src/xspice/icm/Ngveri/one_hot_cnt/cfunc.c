...
extern void cm_one_hot_cnt(Mif_Private_t *);
/* This is cfunc.mod file auto generated by gen_con_info.py
        ...
        #include <math.h>
        #include <string.h>
        #include "sim_main_one_hot_cnt.h"

        
void cm_one_hot_cnt(Mif_Private_t *mif_private) 
{
    ...
    {   
        ...
        fooone_hot_cnt(0,inst_count);
        /* Allocate storage for output ports and set the load for input ports */

        
        port_enable=mif_private->conn[0]->size;

        port_clk=mif_private->conn[1]->size;

        port_reset=mif_private->conn[2]->size;

        port_out=mif_private->conn[3]->size;
    ...
    for(Ii=0;Ii<mif_private->conn[3]->size;Ii++)
    {
        ...
        {
            ((Digital_t*)(mif_private->conn[3]->port[Ii]->output.pvalue))->state = _op_out[Ii];
            }
        else if(_op_out[Ii] != _op_out_old[Ii])
        {
            ((Digital_t*)(mif_private->conn[3]->port[Ii]->output.pvalue))->state = _op_out[Ii];
            mif_private->conn[3]->port[Ii]->delay = ((_op_out[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
            }
        else
        {

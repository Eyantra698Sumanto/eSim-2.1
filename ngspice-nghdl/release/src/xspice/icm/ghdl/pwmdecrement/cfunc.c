...
extern void cm_pwmdecrement(Mif_Private_t *);
...
void cm_pwmdecrement(Mif_Private_t *mif_private) 
{
	...
	char temp_D[1024];
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/pwmdecrement/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
	...
	snprintf(send_data,sizeof(send_data),"C:%s,D:%s", temp_C,temp_D);
			...
			{
        				((Digital_t*)(mif_private->conn[2]->port[Ii]->output.pvalue))->state = _op_Q[Ii];
        			}
			else if(_op_Q[Ii] != _op_Q_old[Ii])
        			{
				((Digital_t*)(mif_private->conn[2]->port[Ii]->output.pvalue))->state = _op_Q[Ii];
        				mif_private->conn[2]->port[Ii]->delay = ((_op_Q[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
        			}
			else
			{

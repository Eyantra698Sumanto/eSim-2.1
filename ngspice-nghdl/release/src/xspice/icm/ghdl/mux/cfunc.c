...
extern void cm_mux(Mif_Private_t *);
...
void cm_mux(Mif_Private_t *mif_private) 
{
	...
	char temp_S1[1024];
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/mux/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
	...
	snprintf(send_data,sizeof(send_data),"A:%s,B:%s,C:%s,D:%s,S0:%s,S1:%s", temp_A,temp_B,temp_C,temp_D,temp_S0,temp_S1);
			...
			{
        				((Digital_t*)(mif_private->conn[6]->port[Ii]->output.pvalue))->state = _op_Z[Ii];
        			}
			else if(_op_Z[Ii] != _op_Z_old[Ii])
        			{
				((Digital_t*)(mif_private->conn[6]->port[Ii]->output.pvalue))->state = _op_Z[Ii];
        				mif_private->conn[6]->port[Ii]->delay = ((_op_Z[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
        			}
			else
			{

...
void cm_attiny_85_nghdl(Mif_Private_t *mif_private) 
{
	...
	char temp_PB5[1024];
		...
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/attiny_85_nghdl/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
	...
	snprintf(send_data,sizeof(send_data),"VCC:%s,GND:%s,clk:%s,PB2:%s,PB3:%s,PB4:%s,PB5:%s", temp_VCC,temp_GND,temp_clk,temp_PB2,temp_PB3,temp_PB4,temp_PB5);
			...
			{
        				((Digital_t*)(mif_private->conn[7]->port[Ii]->output.pvalue))->state = _op_PB0[Ii];
        			}
			else if(_op_PB0[Ii] != _op_PB0_old[Ii])
        			{
				((Digital_t*)(mif_private->conn[7]->port[Ii]->output.pvalue))->state = _op_PB0[Ii];
        				mif_private->conn[7]->port[Ii]->delay = ((_op_PB0[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
        			}
			else
			{
	...
	/* Scheduling event and processing them */
			...
			else
			{
				...
				printf("Client-Unknown value return \n");
        			...
        			if(mif_private->circuit.anal_type == DC)
			{
        				((Digital_t*)(mif_private->conn[8]->port[Ii]->output.pvalue))->state = _op_PB1[Ii];
        			}
			else if(_op_PB1[Ii] != _op_PB1_old[Ii])
        			{
				((Digital_t*)(mif_private->conn[8]->port[Ii]->output.pvalue))->state = _op_PB1[Ii];
        				mif_private->conn[8]->port[Ii]->delay = ((_op_PB1[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
        			}
			else
			{

...
extern void cm_nand_gate(Mif_Private_t *);
...
void cm_nand_gate(Mif_Private_t *mif_private) 
{
	...
	snprintf(send_data,sizeof(send_data),"a:%s,b:%s", temp_a,temp_b);
			...
			{
        				((Digital_t*)(mif_private->conn[2]->port[Ii]->output.pvalue))->state = _op_c[Ii];
        			}
			else if(_op_c[Ii] != _op_c_old[Ii])
        			{
				((Digital_t*)(mif_private->conn[2]->port[Ii]->output.pvalue))->state = _op_c[Ii];
        				mif_private->conn[2]->port[Ii]->delay = ((_op_c[Ii] == ZERO) ? mif_private->param[3]->element[0].rvalue : mif_private->param[2]->element[0].rvalue);
        			}
			else
			{

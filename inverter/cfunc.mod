...
void cm_inverter(ARGS) 
{
	Digital_State_t *_op_o, *_op_o_old;
		...
		_op_o = _op_o_old = (Digital_State_t *) cm_event_get_ptr(0,0);
                ...
                STATIC_VAR(my_ip) = "127.0.0.1";
                my_ip = "127.0.0.1";
                
		char command[1024];
		snprintf(command,1024,"/home/sumanto/ngspice-nghdl/src/xspice/icm/ghdl/inverter/DUTghdl/start_server.sh %d %s &", sock_port, my_ip);
		system(command);
	}
	else
	{
	...
	snprintf(send_data,sizeof(send_data),"i:%s", temp_i);

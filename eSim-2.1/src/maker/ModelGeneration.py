    ...
    def verilogParse(self):
        ...
        if m.name.lower()!=self.fname.split(".")[0] :
            QtWidgets.QMessageBox.critical(
                    None, "Error Message",
                    "<b>Error: File name and module name are not same. Please ensure that they are same</b>",
                    QtWidgets.QMessageBox.Ok 
                )
            
            self.obj_Appconfig.print_info('NgVeri Stopped due to File name and module name not matching error')
            return "Error" 
...
''')
    ...
    def sim_main_header(self):
        ...
        print("Building content for sim_main_"+ self.fname.split('.')[0]+".h file")
    ...
    def sim_main(self):
        ...
        print("Building content for sim_main_"+self.fname.split('.')[0]+".cpp file")
...
'''
        ...
        for i,item in enumerate(self.input_port):
            ...
            before_eval.append('''\t\t\t\t'''+\
                self.fname.split('.')[0]+'''[count]->'''+item.split(':')[0]+''' = arr2int'''+ self.fname.split('.')[0]+'''(temp_'''+item.split(':')[0]+", port_"+item.split(':')[0]+''');\n''')
        ...
        for i,item in enumerate(self.output_port):   
            after_eval.append("\t\t\t\tint2arr"+ self.fname.split('.')[0]+"("+self.fname.split('.')[0]+'''[count] -> '''+\
    ...
    def copy_verilator(self):
        ...
        if os.path.exists(path_icm+"sim_main_"+self.fname.split('.')[0]+".o"):
            os.remove(path_icm+"sim_main_"+self.fname.split('.')[0]+".o")
        if os.path.exists(self.release_home+"src/xspice/icm/"+"verilated.o"):

...
function installNgspice
{
        ...
        chmod +x ../configure
        sudo ../configure --enable-xspice --disable-debug  --prefix=$HOME/$ngspice/install_dir/ --exec-prefix=$HOME/$ngspice/install_dir/
                        
        # Adding patch to Ngspice base code
        sudo cp $src_dir/src/outitf.c $HOME/$ngspice/src/frontend
        sudo cp $src_dir/src/verilated.o $HOME/$ngspice/release/src/xspice/icm/
